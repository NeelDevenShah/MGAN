...
batch_norm = partial(tf.keras.layers.BatchNormalization(), decay=0.9, update_collections=None, epsilon=1e-5, scale=True)

# Here, the z_batch is hte batch of the noise vectors that are used to generate images. The noise vectors are randomly generated and are used to initialize the generators. They are used to create the real images
...
class MGAN(object):
    ...
    def __init__(self):
        ...
        self.tf_session = tf.compat.v1.Session(config=self.tf_config, graph=self.tf_graph)    
    ...
    def _build_model(self):
        ...
        self.x = tf.compat.v1.placeholder(tf.float32, [None, self.img_size[0], self.img_size[1], self.img_size[2]], name="real_data")
        
        self.z = tf.compat.v1.placeholder(tf.float32, [self.g_batch_size * self.num_gens, self.num_z], name='noise')
        
        # Create generator G
        self.g = self._create_generator(self.z)
        
        # Create sampler to generate samples
        self.sampler =  self._create_generator(self.z, train=False, reuse=False)
        
        # The g_bin_loss in the implementation of the MGAN is a binary cross entropy loss function that is used to train the genrator. 
        
        # The g_min_loss is a wasserstein loss funcrion
        ...
        # tf_ones_like = Creates a tensor of all ones that has the same shape as the input.
        self.d_bin_x_loss = tf.reduce_mean(
            # Measures the probability error in tasks with two outcomes in which each outcome is independent and need not have a fully certain label. For instance, one could perform a regression where the probability of an event happening is known and used as a label.
            tf.nn.sigmoid_cross_entropy_with_logits(
                logits=d_bin_g_logits, labels=tf.ones_like(d_bin_g_logits)), name='d_bin_x_loss'
        )
        
        self.d_bin_g_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                logits=d_bin_g_logits, labels=tf.zeros_like(d_bin_g_logits)), name='d_bin_loss'
        )
        
        self.d_bin_loss = tf.add(self.d_bin_x_loss, self.d_bin_g_loss, name='d_bin_loss')
        
        self.d_mul_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=d_mul_g_logits, labels=d_mul_labels), name="d_mul_loss")
        
        self.d_los = tf.add(self.d_bin_loss, self.d_mul_loss, name='d_loss')
        ...
        # Create Optimizers
        self.d_opt = self._create_optimizer(self.d_los, scope='discriminator', lr=self.learning_rate)
        
        self.g_opt = self._create_optimizer(self.g_loss, scope='generator', lr=self.learning_rate)
    ...
    def _create_generator(self, z, train=True, reuse=False, name="generator"):
        ...
        with tf.compat.v1.variable_scope(name)as scope:
            ...
            for i, var in enumerate(z_split):
                h0.append(tf.nn.relu(batch_norm(linear(var, out_size[0][0] * out_size[0][1] * out_size[0][2], scope='g_h0_linear}{}'.format(i), stddev=0.02), is_training=train), scope="g_h0_bn{}".format(i)), name="g_h0_relu{}".format(i))
                
            h = []
            for var in h0:
                h.append(tf.reshape(var, [self.g_batch_size, out_size[0][0] * out_size[0][1] * out_size[0][2]]))
            f = tf.concat(h, axis=0, name='g_h0_relu')
    ...
    def _create_optimizer(self, loss, scope, lr):
        ...
        params = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_RESOURCE_VARIABLES, scope=scope)
        ...
        while(self.epoch < self.num_epochs):
            for batch_idx, (batch_start, batch_end) in enumerate(batches):
                ...
                d_bin_loss, d_mul_loss, d_loss, _ = self.tf_session.run(
                    [self.d_bin_loss, self.d_mul_loss, self.d_los, self.d_opt],
    ...
    def _generate(self, num_samples=100):
        ...
        for batch_idx, (batch_start, batch_end) in enumerate(batches):
            ...
            x[batch_start:batch_end] = sess.run(self.sampler, feed_dict={self.z: z_batch})
    ...
    def _samples_by_gen(self, filepath):
        ...
        x = np.zeros([num_samples, self.img_sizep[0], self.img_sizep[1], self.img_sizep[2]],
                     dtype=np.float32)
        ...
        for i in range(0, img_per_gen, self.g_batch_size):
            ...
            for gen in range(self.num_gens):
                x[gen * img_per_gen + i:gen * img_per_gen + min(i +self.g_batch_size, img_per_gen)] = samples[gen * self.g_batch_size:gen * self.g_batch_size + min(self.g_batch_size, img_per_gen)]
